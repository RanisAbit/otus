# Инструкция по развертывание backend серверов
...
4. Запустить скрипт
```./sql-master.sh```
5. Во время выполенения скрипта нужно создать пароль для пользвателя repl, `вводимы пароль не будет отображаться`.
