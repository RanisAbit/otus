...
1. Запустить скрипт
```./sql-master.sh```
1. Во время выполенения скрипта нужно создать пароль для пользвателя repl, `вводимый пароль не будет отображаться`.
2. После выполнения скрипта будет отображен статус master сервера.
